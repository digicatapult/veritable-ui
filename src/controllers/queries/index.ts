...
export class QueriesController extends HTMLController {
  ...
  public async bavResponseVerify(@Path() queryId: UUID) {
    const query = await this.getQuery(queryId)
    ...
    const { score, description } = await this.bavApi.validate(bavResponse)
    await this.db.update('query', { id: query?.id }, { response: { ...bavResponse, score, description } })

    return this.html(this.responseTemplates.bavVerificationResults({ score, description }))
  }
