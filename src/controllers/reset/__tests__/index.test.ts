  ...
  describe('/reset ', () => {
    ...
    describe('if DEMO_MODE = true', () => {
      ...
      it('deletes connections and connection_invites locally', () => {
        ...
        expect(cloudagentMock.closeConnection.firstCall.args).to.deep.equal(['some-agent-id-1'])
        expect(cloudagentMock.closeConnection.secondCall.args).to.deep.equal(['some-agent-id-2'])
        expect(cloudagentMock.closeConnection.thirdCall.args).to.deep.equal(['some-agent-id-3'])
