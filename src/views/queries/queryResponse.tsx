...
export default class QueryResponseTemplates {
    ...
    return (
      <div class="container-query-form">
        ...
        <div class="query-form-right">
          ...
          <this.bavVerificationResults score={responseData.score} description={responseData.description} />
          <LinkButton text="Back to Queries" href="/queries" style="filled" />
        </div>
      </div>
    )
  ...
  public bavVerificationResults = ({ score, description }: { score?: number; description?: string }): JSX.Element => {
    return (
      <table id="bav-verification-results" class="query-response-view">
        <div id="spinner" />
        <div id="bav-verification-results-details">
          {score && description && (
            <>
              <tr>
                <td>Score:</td>
                <td class="query-results-left-padding-table">{Html.escapeHtml(score)}</td>
              </tr>
              <tr>
                <td>Description:</td>
                <td class="query-results-left-padding-table">
                  {Html.escapeHtml(description)}
                  <span class={`bav-verify-icon ${score >= 0.95 ? 'tick' : score >= 0.5 ? 'tilde' : 'cross'}`} />
                </td>
              </tr>
            </>
          )}
