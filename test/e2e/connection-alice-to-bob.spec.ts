...
import { aliceE2E, bobE2E } from '../helpers/setupConnection.js'
...
test.describe('Connection from Alice to Bob', () => {
  ...
  test('Connection from Alice to Bob', async () => {
    await test.step('Alice invites Bob to connect', async () => {
      ...
      await expect(page.locator('#new-invite-country-code-display')).toHaveValue(bobE2E.registryCountryCode)

      await page.locator('#new-invite-company-number-input').pressSequentially(bobE2E.companyNumber, { delay: 20 })
      ...
      await expect(feedbackElement).toContainText(bobE2E.companyName)
      ...
      await expect(confirmationElement).toContainText(`Company Number: ${bobE2E.companyNumber}`)
    ...
    await test.step('Bob submits invite and pin', async () => {
      if (!invite) throw new Error('Invitation for Bob was not found.')
      await page.goto(`${bobE2E.url}/connection`, { waitUntil: 'networkidle' })
      ...
      // Submit pin
      await page.waitForLoadState('networkidle')
      await page.locator('#new-connection-invite-input-pin').pressSequentially(pinForBob, { delay: 20 })
      const bobButton = page.locator('button[type="submit"][name="action"][value="submitPinCode"]')
      await expect(bobButton).toBeVisible()
      await bobButton.click({ delay: 50 })
    ...
    await test.step('Check connection is in state verified', async () => {
      ...
      await expect(page.locator('#search-results')).toContainText(bobE2E.companyNumber)
    })
  })
})
